...
__global__ void ini_size(int *x) {
    dsize = x[0];
}
...
__global__ void maintain_wall(float *data) {
    ...
    for (int i = dsize * (dsize - 1); i < dsize * dsize; i++) {
        data[i] = wall_temp;
    }
    for (int i = 0; i < dsize; i++) {
        data[dsize * i] = wall_temp;
        data[dsize * i + dsize - 1] = wall_temp;
    }
}
...
int main(int argc, char *argv[]){
    ...
    int* temp1 = &size;
    int* temp2;
    cudaMalloc(&temp2, sizeof(int));
    cudaMemcpy(temp2, temp1, sizeof(int), cudaMemcpyHostToDevice);
    ini_size<<<1, 1>>>(temp2);
